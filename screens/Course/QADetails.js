...
const qa_details_tabs = constants.qa_details_tabs.map((tab) => ({
    ...tab,
    ref: createRef()
}))
...
const TabIndicator = ({measureLayout, scrollX}) => {
    const inputRange = qa_details_tabs.map((_, i) => i * SIZES.width);
...
const Tabs = ({scrollX, onTabPress}) => {
    ...
    useEffect(() => {
        ...
        qa_details_tabs.forEach(course_details_tab => {
            course_details_tab?.ref?.current?.measureLayout(
                containerRef.current,
                (x, y, width, height) => {
                    ml.push({x, y, width, height});
                    if (ml.length === qa_details_tabs.length) {
                        setMeasureLayout(ml);
                    }
                })
        })
    ...
    return (
        <View ref={containerRef} style={{flex: 1, flexDirection: "row", backgroundColor: COLORS.primary3, borderBottomWidth: 1, borderBottomColor: COLORS.primary3}}>
            ...
            { /* Tabs */ }
            {qa_details_tabs.map((item, index) => {
                return (
                    <TouchableOpacity
                        key={`Tab-${index}`}
                        ref={item.ref}
    ...
    return (
        <FlatList
            ...
            renderItem={({ item, index }) => {
                return (
                    <TouchableOpacity onPress={() => onPress(`${item.id}`)} style={{backgroundColor: COLORS.white, paddingVertical: SIZES.padding, borderBottomWidth: 1, borderBottomColor: COLORS.gray10}}>
...
const QADetails = ({navigation, route}) => {

    const { selectedCategory, initialArticle } = route.params;
    const [article, setArticle] = useState({notes: []})
    const [articleId, setArticleId] = useState(() => (initialArticle ? initialArticle.id : selectedCategory.articles[0].id))
        ...
        return (
            <View style={{flex: 1}}>
                ...
                <Animated.FlatList
                    ...
                    data={constants.qa_details_tabs}
